  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  @Input() animationName: string | null = "fadeIn"; // use fadeIn as default if not specified, specify null for no animation
  ...
  @Input() intersectOffset: string | null = "0px";
  @Input() navTitle: string | null = "0px";
  ...
  ngOnInit(): void {
	...
	this.intersectOffset = this.intersectOffset ? this.intersectOffset || "0px" : "0px";
	this.navTitle = this.navTitle ? this.navTitle || '' : '';
    ...
    if ("IntersectionObserver" in window && this.useScroll) {
      const options: IntersectionObserverInit = {
        ...
        rootMargin: "0px 0px " + this.intersectOffset + " 0px",
      };
  ...
  private addAnimationClass(): void {
	...
	if (this.animationName === 'updateNav') {
	  this.app.setNavTitle(this.navTitle!, this.elementRef.nativeElement.id);
	} else {
